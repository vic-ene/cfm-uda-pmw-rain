...
from model__it_utils._it_utils import modulate, TimestepEmbedder, LabelEmbedder
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        if self.num_classes is not None:
            if self.cond_used in ["vanilla", "cfg"]:
                self.y_embedder = LabelEmbedder(num_classes, time_embed_dim, self.class_dropout_prob, cond_used=self.cond_used)
            else:
                sys.exit("I do not recognise the conditioning used")
    ...
    def forward(self, t, x, y=None):
        ...
        if self.y_embedder != None:
            emb = emb + self.y_embedder(y, self.training)
