...
class ConditionalFlowMatcher:
    ...
    def __init__(self, sigma: Union[float, int] = 0.0, noise_type="gaussian", extra_std=1.0):
    ...
    def sample_noise_like(self, x):
        if self.noise_type == "gaussian":
            ...
            return torch.randn_like(x).uniform_(-1.0, 1.0)
        elif self.noise_type == "anti":
            return -x.detach().clone()
        elif self.noise_type == "anti_and_noise":
            return -x.detach().clone() + torch.randn_like(x) * self.extra_std


    def sample_location_and_conditional_flow(self, x0, x1, t=None, return_noise=False, logit_normal_mu=0.0, logit_normal_sigma=0.0):
        """
        ...
        do_logit_normal = (logit_normal_sigma != 0.0)

        if t is None:
            if not do_logit_normal:
                t = torch.rand(x0.shape[0]).type_as(x0)
            else:
                noise = logit_normal_mu + torch.randn(
                    x0.shape[0],
                    device=x0.device,
                    dtype=x0.dtype
                ) * logit_normal_sigma
                t = torch.sigmoid(noise)

 
        assert len(t) == x0.shape[0], "t has to have batch size dimension"

        if self.noise_type == "anti":
            x0 = -x1.clone().detach()
